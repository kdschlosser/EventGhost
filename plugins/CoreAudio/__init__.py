...
class Text(eg.TranslatableStrings):
    ...
    class GetEndpointVolume:
        name = 'GetEndpointVolume'
        description = 'Gets the volume level for a given endpoint.'

    class SetEndpointVolume:
        name = 'SetEndpointVolume'
...
class Callbacks:
    ...
    def __init__(self, plugin):
        ...
        self.__default_endpoints = dict(Render=None, Capture=None)
            ...
            for endpoint in device:
                try:
                    ...
                    self.__endpoints[endpoint] = dict(
                        ...
                        mute=volume.mute,
                        channels=volume.channel_levels_scalar
                    )
    ...
    def endpoint_volume_change(
        ...
        old_vol = self.__endpoints[endpoint]['vol']
        if old_vol != new_vol:
            self.__endpoints[endpoint]['vol'] = new_vol
            self.plugin.TriggerEvent(
                '{0}.{1}.Volume.{2:.4f}'.format(
                    endpoint.device.name,
                    self.__format_endpoint_name(endpoint),
                    new_vol * 100.0
                ),
        ...
        old_mute = self.__endpoints[endpoint]['mute']
        if old_mute != new_mute:
            self.__endpoints[endpoint]['mute'] = new_mute
            if new_mute:
                self.plugin.TriggerEvent(
                    '{0}.{1}.MuteOn'.format(
                        endpoint.device.name,
        ...
        old_channels = self.__endpoints[endpoint]['channels']
        ...
        if old_channels != new_channels:
            ...
            for i, channel in enumerate(new_channels):
                if channel > new_vol:
                    high_channels += [[i, channel]]
                elif channel < new_vol:
                    low_channels += [[i, channel]]
                else:
                    equal_channels += [[i, channel]]

            if len(low_channels) == 1:
                for i, channel in low_channels:
                    self.plugin.TriggerEvent(
                        '{0}.{1}.Channel.{2}.Volume.{3:.4f}'.format(
                            endpoint.device.name,
                            self.__format_endpoint_name(endpoint),
                            i,
                            channel * 100.0
            ...
            if len(equal_channels) == 1:
                for i, channel in equal_channels:
                    self.plugin.TriggerEvent(
                        '{0}.{1}.Channel.{2}.Volume.{3:.4f}'.format(
                            endpoint.device.name,
                            self.__format_endpoint_name(endpoint),
            ...
            self.__endpoints[endpoint]['channels'] = new_channels
    ...
    def device_added(self, device):
        self.plugin.TriggerEvent(
            'Device.{0}.Added'.format(device.name),
            device
        )
        self.__registered_endpoints[device] = []
    ...
    def device_removed(self, device):
        ...
        self.plugin.TriggerEvent('Device.{0}.Removed'.format(device.name))

    def default_endpoint_changed(self, device):
        for endpoint in device:
            if endpoint.is_default:
                if self.__default_endpoints[endpoint.data_flow] != endpoint:
                    self.plugin.TriggerEvent(
...
class CoreAudio(eg.PluginBase):
    ...
    def __init__(self):
        ...
        self.AddAction(SetEndpointVolume)
        self.AddAction(GetSessionVolume)
        self.AddAction(GetEndpointChannelVolume)
        self.AddAction(SetEndpointChannelVolume)
...
class DevicePanel(wx.Panel):
    ...
    def on_choice(self, evt=None):
        ...
        if device in ('Default Playback Device', 'Default Recording Device'):
            ...
            self.connector_ctrl.SetLabel(str(device.connector_count))
            self.state_ctrl.SetLabel(str(device.state))
...
class EndpointPanel(wx.Panel):
    ...
    def on_choice(self, evt=None):
        ...
        self.description_ctrl.SetLabel(str(endpoint.description))
        self.data_flow_ctrl.SetLabel(str(endpoint.data_flow))
        self.form_factor_ctrl.SetLabel(str(endpoint.form_factor))
        self.full_speaker_ctrl.SetLabel(str(endpoint.full_range_speakers))
        self.guid_ctrl.SetLabel(str(endpoint.guid))
        self.physical_speaker_ctrl.SetLabel(str(endpoint.physical_speakers))
        self.effects_ctrl.SetLabel(str(endpoint.system_effects))
...
class SessionPanel(wx.Panel):
    ...
    def on_choice(self, evt=None):
        ...
        self.state_ctrl.SetLabel(str(session.state))
        if evt is not None:
            evt.Skip()
...
class VolumeInfoCtrl(wx.Panel):
    ...
    def set_endpoint(self, endpoint):
        if endpoint:
            ...
            self.scalar_volume_ctrl.SetLabel('{0:.4f}'.format(scalar))
            self.scalar_min_ctrl.SetLabel('{0:.4f}'.format(scalar_min))
            self.scalar_max_ctrl.SetLabel('{0:.4f}'.format(scalar_max))
            self.scalar_step_ctrl.SetLabel('{0:.4f}'.format(scalar_step))

            self.db_volume_ctrl.SetLabel('{0:.4f}'.format(db))
            self.db_min_ctrl.SetLabel('{0:.4f}'.format(db_min))
            self.db_max_ctrl.SetLabel('{0:.4f}'.format(db_max))
            self.db_step_ctrl.SetLabel('{0:.4f}'.format(db_step))
        else:
...
class SetEndpointVolume(eg.ActionBase):
    ...
    def __call__(self, device, endpoint, volume):
        if device == 'Default Playback Device':
            ...
            for dev in pyWinCoreAudio.AudioDevices:
                ...
                return None
        try:
            endpoint.volume.master_scalar = volume / 100.0
            return endpoint.volume.master_scalar * 100.0
        except AttributeError:
            return None
...
class SetEndpointChannelVolume(eg.ActionBase):
    ...
    def __call__(self, device, endpoint, channel, volume):
        if device == 'Default Playback Device':
            ...
            for dev in pyWinCoreAudio.AudioDevices:
                ...
                return None
        try:
            endpoint.volume.channels[channel].level_scalar = volume / 100
...
class GetEndpointMute(eg.ActionBase):
    ...
    def Configure(
        ...
        if endpoint is not None:
            try:
                vol = endpoint.volume
                mute_info_ctrl.SetLabel(str(vol.mute))
            except AttributeError:
                panel.EnableButtons(False)
            ...
            if deviceId == 'Primary Sound Driver':
                ...
                for device in pyWinCoreAudio.AudioDevices:
                    ...
                    raise SoundMixer.SoundMixerException(
                        'Device {0} not found.'.format(deviceId)
                    )
